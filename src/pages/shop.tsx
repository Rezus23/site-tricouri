...
import { FaFire } from "react-icons/fa"; // Iconiță pentru titlu nou
...
export default function Shop() {
  ...
  // LISTA CATEGORII (Asigură-te că ID-ul pentru precomandă e corect, aici am pus 'precomanda')
  const categories = [
    { id: "all", label: "Toate Produsele" },
    { id: "tricouri", label: "Sezon 24/25" },
    { id: "retro", label: "Retro" },
    { id: "nationale", label: "Echipe Naționale" },
    { id: "precomanda", label: "Precomandă" }, // 👈 Categoria ta
  ...
  // --- FUNCȚIE RENDERIZARE CARD (Ca să nu duplicăm codul HTML) ---
          ...
          <Link href={`/magazin/${produs.id}`} className="block relative aspect-[4/5] bg-[#f4f4f5] overflow-hidden">
            ...
            {isSoldOut && (
                <div className="absolute inset-0 flex items-center justify-center z-30 bg-black/10">
                    <span className="bg-red-600 text-white px-4 py-2 font-black text-lg uppercase tracking-widest transform -rotate-12 shadow-xl border-2 border-white">Sold Out</span>
                </div>
            )}
            ...
            {!isSoldOut && (
                <div className="absolute bottom-4 left-4 right-4 translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-20">
                    <button className="w-full bg-white text-black font-bold py-3 rounded-xl shadow-xl hover:bg-black hover:text-white transition-colors text-xs uppercase tracking-widest">Vezi Detalii</button>
                </div>
            )}
          </Link>
          ...
          <div className="p-5 bg-white relative z-10">
            <div className="flex justify-between items-start gap-4 mb-1">
                <Link href={`/magazin/${produs.id}`}>
                    <h3 className="font-bold text-md text-gray-900 leading-tight group-hover:text-blue-600 transition-colors line-clamp-2 uppercase">{produs.titlu}</h3>
                </Link>
                <p className="font-black text-lg text-gray-900 whitespace-nowrap">{produs.pret} <span className="text-[10px] text-gray-500 align-top font-normal">RON</span></p>
            </div>
            {isSoldOut ? (
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center relative bg-black">
        <BlurredBackground />
        <p className="text-xl text-white font-bold drop-shadow-md animate-pulse">Se încarcă colecția...</p>
      </div>
    );
  }
      ...
      <div className="max-w-[1400px] mx-auto p-6 relative z-10 pt-10">
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center mt-20 p-10 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 max-w-lg mx-auto">
            ...
            <Link href="/contact" className="inline-block bg-white text-black px-6 py-3 rounded-full font-bold hover:bg-gray-200 transition">Cere un produs</Link>
          </div>
        ) : (
          <>
            {/* 👇 AICI ESTE LOGICA DE SEPARARE PENTRU PRECOMANDĂ 👇 */}
            {activeCategory === 'precomanda' ? (
                <>
                    {/* 1. Primele 2 produse */}
